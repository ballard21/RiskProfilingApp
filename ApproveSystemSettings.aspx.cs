...
{
    public partial class ApproveSystemSettings : System.Web.UI.Page
    {
        ...
        private void InitializeControls(string user, string token)
        {
            ...
            try
            {
                ...
                if (tables.Rows.Count != 0)
                {
                    ...
                    DataSet dset = handler.GetData("getAllMakerChanges", new object[] { fullname });
                    handler.InsertLoginLog("Accessed Checker to Approve or Reject changes Page", _loggedUser);
                    ...
                    {
                        lblmsg.Text = "No Items to Verify";
                        var jsonResponse = new JavaScriptSerializer().Serialize(new
                        {
                            ErrorCode = "100",
            ...
            {
                handler.InsertLoginLog("ApproveSystemSettings Error: InitializeControls  :", ee.Message +" User : "+fullname);
            }
        }
        ...
        {
            string _loggedUser = Session["fullname"].ToString();
            handler.InsertLoginLog("Downloading User List by Admin", _loggedUser);
            string fileName = "UserReport";
            string filePath = @"C:\Users\public\downloads\" + fileName + ".csv";
            DataTable dset = handler.GetDataFromAppDT("GetUsersToEnable", new object[] { });
            ToCSV(dset, filePath);
        }
